...
class R_CNNCritic(nn.Module):
    ...
    :param obs_space: (dict) observation space of the environment.
    :param cent_obs_space: (dict) centralized observation space of the environment.
    :param action_space: (dict) action space of the environment.
...
class R_CNNActor(nn.Module):
    ...
    :param device: (torch.device) device to place network on.
    """
...
class CNNActorAndCritic(nn.Module):
    """
    Actor-Critic network for MADDPG.
    :param args: (argparse.Namespace) arguments containing relevant model parameters.
    :param obs_space: (gym.Space) observation space.
